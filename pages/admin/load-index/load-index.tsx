import { LoadIndexProps } from "@Store/tyre/types";
import Link from "next/link";
import React, { useEffect, useState } from "react";

const LoadIndex = ({ loadIndexes, getLoadIndexes }: LoadIndexProps) => {
  ...
  const filtered = loadIndexes?.filter((loadIndex) =>
    loadIndex.value
      .toString()
      .toLowerCase()
      .replace(/\s+/g, "")
  ...
  return (
    <div>
      ...
      <div className="w-full flex pt-1 justify-end">
        <Link href="/admin/load-index/create">
          ...
          <tbody>
            {filtered?.map((loadIndex, index) => (
              ...
              >
                ...
                <td className="px-6 py-4">{loadIndex.value}</td>

                <td className="px-6 py-4 text-right">
                  <Link
                    href={{
                      pathname: "/admin/load-index/update",
                      query: {
                ...
                <td className="px-6 py-4 text-right">
                  <Link
                    href={{
                      pathname: "/admin/load-index/delete",
                      query: {
                        loadIndexId: loadIndex.id,
                        loadIndexValue: loadIndex.value,
                      },
