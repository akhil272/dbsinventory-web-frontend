...
import { UpdateVehicleModelProps } from "@Store/adminPanel/types";
...
const Update = ({ updateVehicleModel }: UpdateVehicleModelProps) => {
  ...
  const updateExistingTransport = async (data: GenericFormData) => {
    const response = await updateVehicleModel({
      id: Number(vehicleModelId),
      model: data.name,
    });
    if (response.success && response.data) {
      toast.success(`Successfully updated vehicle brand name to ${data.name}.`);
      router.back();
    }
    if (!response.success) {
    ...
    <div className="pb-4">
      ...
      <form className="space-y-2  pt-6" onSubmit={onSubmit}>
        <InputField
          ...
          defaultValue={String(vehicleModel) ?? "N/A"}
        />
        <button
