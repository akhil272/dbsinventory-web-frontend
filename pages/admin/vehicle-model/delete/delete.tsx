import { DeleteVehicleModelProps } from "@Store/adminPanel/types";
import { useRouter } from "next/router";
import { toast } from "react-toastify";

const Delete = ({ deleteVehicleModel }: DeleteVehicleModelProps) => {
  const router = useRouter();
  const {
    query: { vehicleModelId, vehicleModel },
  } = router;
  const cancel = () => {
    router.back();
  };
  const confirmDelete = async () => {
    const response = await deleteVehicleModel({ id: Number(vehicleModelId) });
    if (response.success) {
      toast.success(`${vehicleModel} deleted successfully.`);
      router.back();
    }
    if (!response.success) {
      toast.error(`Failed to delete ${vehicleModel}. ${response.message}`);
    }
  };
