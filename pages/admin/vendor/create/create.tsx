...
import { CreateVendorProps } from "@Store/adminPanel/types";
...
const Create = ({ createVendor }: CreateVendorProps) => {
  const {
    handleSubmit,
    control,
    reset,
  ...
  const onSubmit = handleSubmit((data) => createNewVendor(data));

  const createNewVendor = async (data: GenericFormData) => {
    ...
    if (!response.success) {
      toast.error(`Failed to vendor to system. ${response.message}`);
    }
  };
