import { DeleteVendorProps } from "@Store/adminPanel/types";
import { useRouter } from "next/router";
import { toast } from "react-toastify";

const Delete = ({ deleteVendor }: DeleteVendorProps) => {
  ...
  const confirmDelete = async () => {
    const response = await deleteVendor({ id: Number(vendorId) });
    if (response.success) {
      toast.success(`${vendorName} deleted successfully.`);
      router.back();
    }
    if (!response.success) {
      toast.error(`Failed to delete ${vendorName}. ${response.message}`);
    }
  };
