...
import { UpdateVendorProps } from "@Store/adminPanel/types";
...
const Update = ({ updateVendor }: UpdateVendorProps) => {
  const router = useRouter();
  const {
    query: { vendorId, vendorName },
  } = router;
  const {
    handleSubmit,
    control,
  ...
  const onSubmit = handleSubmit((data) => updateExistingVendor(data));

  const updateExistingVendor = async (data: GenericFormData) => {
    const response = await updateVendor({
      id: Number(vendorId),
      name: data.name,
    });
    if (response.success && response.data) {
      toast.success(`Successfully updated vendor name to ${data.name}.`);
      router.back();
    }
    if (!response.success) {
    ...
    <div className="pb-4">
      ...
      <form className="space-y-2  pt-6" onSubmit={onSubmit}>
        <InputField
          ...
          defaultValue={String(vendorName) ?? "N/A"}
        />
        <button
