import { ProductLineProps } from "@Store/tyre/types";
import Link from "next/link";
import React, { useEffect, useState } from "react";

const ProductLine = ({ productLines, getProductLines }: ProductLineProps) => {
  ...
  const filtered = productLines?.filter((productLine) =>
    productLine.name
      .toLowerCase()
      .replace(/\s+/g, "")
  ...
  return (
    <div>
      ...
      <div className="w-full flex pt-1 justify-end">
        <Link href="/admin/product-line/create">
      ...
      <div className="relative overflow-x-auto shadow-md sm:rounded-lg mt-4">
        ...
        <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
          ...
          <tbody>
            {filtered?.map((productLine, index) => (
              <tr
                key={productLine.id}
              ...
              >
                ...
                <td className="px-6 py-4">{productLine.name}</td>

                <td className="px-6 py-4 text-right">
                  <Link
                    href={{
                      pathname: "/admin/product-line/update",
                      query: {
                ...
                <td className="px-6 py-4 text-right">
                  <Link
                    href={{
                      pathname: "/admin/product-line/delete",
                      query: {
                        productLineId: productLine.id,
                        productLineName: productLine.name,
                      },
