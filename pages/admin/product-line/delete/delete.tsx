import { DeleteProductLineProps } from "@Store/tyre/types";
import { useRouter } from "next/router";
import { toast } from "react-toastify";

const Delete = ({ deleteProductLine }: DeleteProductLineProps) => {
  ...
  const confirmDelete = async () => {
    const response = await deleteProductLine({ id: Number(productLineId) });
    if (response.success) {
      toast.success(`${productLineName} deleted successfully.`);
      router.back();
    }
    if (!response.success) {
      toast.error(`Failed to delete ${productLineName}. ${response.message}`);
    }
  };
