import { DeleteServiceProps } from "@Store/quotations/types";
import { useRouter } from "next/router";
import { toast } from "react-toastify";

const Delete = ({ deleteService }: DeleteServiceProps) => {
  ...
  const confirmDelete = async () => {
    const response = await deleteService({ id: Number(serviceId) });
    if (response.success) {
      toast.success(`${serviceName} deleted successfully.`);
      router.back();
    }
    if (!response.success) {
      toast.error(`Failed to delete ${serviceName}. ${response.message}`);
    }
  };
