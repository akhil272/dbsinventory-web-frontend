...
import { UpdateVehicleBrandProps } from "@Store/adminPanel/types";
...
const Update = ({ updateVehicleBrand }: UpdateVehicleBrandProps) => {
  ...
  const updateExistingTransport = async (data: GenericFormData) => {
    const response = await updateVehicleBrand({
      id: Number(vehicleBrandId),
    ...
    <div className="pb-4">
      ...
      <form className="space-y-2  pt-6" onSubmit={onSubmit}>
        <InputField
          ...
          defaultValue={String(vehicleBrand) ?? "N/A"}
        />
        <button
