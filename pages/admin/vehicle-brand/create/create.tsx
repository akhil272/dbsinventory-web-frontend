...
import { CreateVehicleBrandProps } from "@Store/adminPanel/types";
...
const Create = ({ createVehicleBrand }: CreateVehicleBrandProps) => {
  const {
    handleSubmit,
    control,
    reset,
  ...
  const onSubmit = handleSubmit((data) => createNewTransport(data));
  const createNewTransport = async (data: GenericFormData) => {
    const response = await createVehicleBrand({
      vehicleBrand: data.name,
    });
    ...
    if (!response.success) {
      toast.error(`Failed to add vehicle brand to system. ${response.message}`);
    }
  };
