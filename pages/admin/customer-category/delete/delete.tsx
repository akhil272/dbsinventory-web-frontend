import { DeleteCustomerCategoryProps } from "@Store/customers/types";
import { useRouter } from "next/router";
import { toast } from "react-toastify";

const Delete = ({ deleteCustomerCategory }: DeleteCustomerCategoryProps) => {
  ...
  const confirmDelete = async () => {
    const response = await deleteCustomerCategory({
      id: Number(customerCategoryId),
    });
    if (response.success) {
      toast.success(`${customerCategoryName} deleted successfully.`);
      router.back();
    }
    if (!response.success) {
      toast.error(
        `Failed to delete ${customerCategoryName}. ${response.message}`
      );
    }
  };
