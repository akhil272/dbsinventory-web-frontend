...
import { CreateTransportProps } from "@Store/adminPanel/types";
...
const Create = ({ createTransport }: CreateTransportProps) => {
  const {
    handleSubmit,
    control,
    reset,
  ...
  const createNewTransport = async (data: GenericFormData) => {
    ...
    if (!response.success) {
      toast.error(`Failed to add transport to system. ${response.message}`);
    }
  };
