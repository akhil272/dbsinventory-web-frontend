import { SpeedRatingProps } from "@Store/tyre/types";
import Link from "next/link";
import React, { useEffect, useState } from "react";

const SpeedRating = ({ speedRatings, getSpeedRatings }: SpeedRatingProps) => {
  ...
  const filtered = speedRatings?.filter((speedRating) =>
    speedRating.value
      .toLowerCase()
      .replace(/\s+/g, "")
  ...
  return (
    <div>
      ...
      <div className="w-full flex pt-1 justify-end">
        <Link href="/admin/speed-rating/create">
      ...
      <div className="relative overflow-x-auto shadow-md sm:rounded-lg mt-4">
        ...
        <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
          ...
          <tbody>
            {filtered?.map((speedRating, index) => (
              <tr
                key={speedRating.id}
              ...
              >
                ...
                <td className="px-6 py-4">{speedRating.value}</td>

                <td className="px-6 py-4 text-right">
                  <Link
                    href={{
                      pathname: "/admin/speed-rating/update",
                      query: {
                        speedRatingId: speedRating.id,
                        speedRatingValue: speedRating.value,
                      },
                    }}
                  >
                ...
                <td className="px-6 py-4 text-right">
                  <Link
                    href={{
                      pathname: "/admin/speed-rating/delete",
                      query: {
                        speedRatingId: speedRating.id,
                        speedRatingValue: speedRating.value,
                      },
